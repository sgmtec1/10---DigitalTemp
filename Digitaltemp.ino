...
int pinoDigital = 9; //PINO DIGITAL (DETECTA A PRESENÇA DO CAMPO MAGNÉTICO)
...
void loop (){
  ...
  Serial.println("APROXIME-SE"); //IMPRIME O TEXTO NO MONITOR SERIAL
  lcd.setCursor(0,0); //SETA A POSIÇÃO DO CURSOR
  lcd.print("APROXIME-SE");
  val = digitalRead(pinoDigital);

  
  if(val == HIGH){ //SE LEITURA DO PINO FOR IGUAL A HIGH, FAZ
